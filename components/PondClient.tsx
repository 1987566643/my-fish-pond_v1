...
type Bubble = { x: number; y: number; r: number; vy: number; alpha: number; phase: number; };
...
export default function PondClient() {
  ...
  /** 今日我对各鱼的反应：1=赞, -1=踩, 0/undefined=无 */
  const [myReactions, setMyReactions] = useState<Record<string, 1 | -1 | 0>>({});
  ...
  async function refreshAll() {
    ...
    try {
      ...
      setTodayCatchCount(j && j.ok ? (j.today_catch ?? 0) : 0);
    } catch { setTodayCatchCount(0); }

    // 今日我的反应 map
    try {
      const j = await fetch('/api/reaction', { cache: 'no-store' }).then(r => r.json());
      setMyReactions(j?.reactions || {});
    } catch {}
  }
  ...
  function initDrawCanvas() {
    ...
    function drawGuides() {
      ...
      for (let i = 0; i < 6; i++) {
        ctx.beginPath(); ctx.arc(rnd(0, cvs.width), rnd(0, cvs.height), rnd(8, 30), 0, Math.PI * 2);
        ctx.fillStyle = '#9dd0ff'; ctx.fill();
      }
      ctx.restore();
      ctx.save();
      ctx.strokeStyle = 'rgba(255,255,255,.25)'; ctx.setLineDash([6, 6]);
      ctx.beginPath(); ctx.moveTo(16, 20); ctx.lineTo(cvs.width - 16, 20); ctx.stroke(); ctx.restore();
      ...
      for (const s of strokes) {
        ...
        ctx.beginPath();
        for (let i = 0; i < s.points.length; i++) { const p = s.points[i]; i === 0 ? ctx.moveTo(p.x, p.y) : ctx.lineTo(p.x, p.y); }
        ctx.stroke();
      }
    }
    ...
    function move(ev: PointerEvent) {
      ...
      ctx2.strokeStyle = stroke.color; ctx2.lineWidth = stroke.size; ctx2.lineCap = 'round'; ctx2.lineJoin = 'round';
    ...
    (cvs as any).redraw = drawGuides; drawGuides();
  ...
  function clearDrawing() { const cvs = drawCanvasRef.current!; (cvs as any)._strokes = []; (cvs as any).redraw?.(); }
  function undoDrawing() { const cvs = drawCanvasRef.current!; const s = ((cvs as any)._strokes as any[])||[]; if (s.length){ s.pop(); (cvs as any).redraw?.(); } }
  ...
  async function saveFish() {
    ...
    const off = document.createElement('canvas'); off.width = EXPORT_W; off.height = EXPORT_H;
    const g = off.getContext('2d')!;
    for (const s of strokes) {
      g.strokeStyle = s.color; g.lineWidth = s.size; g.lineCap = 'round'; g.lineJoin = 'round';
      g.beginPath(); for (let i = 0; i < s.points.length; i++) { const p = s.points[i]; i===0?g.moveTo(p.x,p.y):g.lineTo(p.x,p.y); } g.stroke();
    }
    ...
    const dg = dst.getContext('2d')!; dg.imageSmoothingEnabled = true; dg.imageSmoothingQuality = 'high';
    dg.drawImage(off, 0, 0, dstW, dstH);

    const data_url = dst.toDataURL('image/png');
    const name = (fishName || '').trim() || `无名鱼-${String(Date.now()).slice(-5)}`;

    const res = await fetch('/api/fish', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, data_url, w: dstW, h: dstH }) });

    if (res.ok) {
      drawDlgRef.current?.close(); setFishName(''); clearDrawing();
      await refreshAll();
  ...
  function drawWater(ctx: CanvasRenderingContext2D, W: number, H: number, ts: number) {
    ...
    ctx.save(); ctx.globalAlpha = 0.06; ctx.lineWidth = 18; ctx.strokeStyle = '#cfeaff';
    const t = ts * 0.001;
    for (let i = 0; i < 5; i++) {
      const baseY = (H * (i + 1)) / 6, amp = 12 + i * 3, freq = 0.8 + i * 0.15;
      ctx.beginPath();
  ...
  function rebuildSprites(list: ServerFish[]) {
    const cvs = pondRef.current!; const W = cvs?.clientWidth || 800; const H = cvs?.clientHeight || 480;
    const prev = spritesRef.current || []; const map = new Map<string, PondSprite>(); for (const p of prev) map.set(p.id, p);
    const next: PondSprite[] = [];
    for (const f of list) {
      const existed = map.get(f.id);
      if (existed) {
        existed.name = f.name; existed.owner_name = f.owner_name; existed.likes = f.likes||0; existed.dislikes = f.dislikes||0; existed.created_at = f.created_at;
        if (existed.data_url !== f.data_url) { existed.data_url = f.data_url; existed.img = new Image(); existed.img.src = f.data_url; }
        existed.w = f.w; existed.h = f.h; next.push(existed);
      } else {
        const img = new Image(); img.src = f.data_url;
        next.push({ id: f.id, name: f.name, owner_name: f.owner_name, data_url: f.data_url, w: f.w, h: f.h, created_at: f.created_at,
          likes: f.likes||0, dislikes: f.dislikes||0, img,
          x: Math.max(80, Math.min(W-80, Math.random()*(W-160)+80)), y: Math.max(80, Math.min(H-80, Math.random()*(H-160)+80)),
  ...
  useEffect(() => {
    const cvs = pondRef.current!; if (!cvs) return;
    setupHiDPI(cvs);
    const ctx = cvs.getContext('2d')!;
    if (bubblesRef.current.length === 0) {
      const N = Math.max(10, Math.floor((cvs.clientWidth * cvs.clientHeight) / 45000));
      for (let i = 0; i < N; i++) {
        bubblesRef.current.push({ x: Math.random()*cvs.clientWidth, y: Math.random()*cvs.clientHeight, r: 3+Math.random()*6,
          vy: 12+Math.random()*16, alpha: 0.12+Math.random()*0.1, phase: Math.random()*Math.PI*2 });
      }
    }
    ...
    function drawHookAndCheck(ctx2: CanvasRenderingContext2D) {
      ...
      const size = HOOK_SIZE; const eyeY = f.y - size;
      ctx2.save(); ctx2.strokeStyle = '#c7e8ff'; ctx2.lineWidth = 2; ctx2.globalAlpha = 0.9;
      ctx2.beginPath(); ctx2.moveTo(f.x, 0); ctx2.lineTo(f.x, Math.max(0, eyeY)); ctx2.stroke(); ctx2.restore();
      ...
      if (!f.caughtId) {
        for (const s of spritesRef.current) {
          ...
          if (Math.hypot(mouthX - f.x, mouthY - f.y) <= f.biteRadius) {
            s.caught = true; s.x = f.x; s.y = f.y; fishingRef.current.caughtId = s.id; break;
          }
        }
      } else {
    ...
    function frame(ts: number) {
      ...
      for (const b of bubblesRef.current) {
        b.y -= b.vy * dt;
        if (b.y + b.r < -10) {
          b.y = H + 20 + Math.random()*40; b.x = Math.random()*W; b.r = 3+Math.random()*6; b.vy = 12+Math.random()*16; b.phase = Math.random()*Math.PI*2;
        }
      }

      // 鱼游动 + 绘制
      for (const s of spritesRef.current) {
        if (!s.caught && !hoverLock) { // 锁定时暂停鱼（更好点按钮）
          s.turn -= dt;
          ...
          if (s.x < margin || s.x > W - margin || s.y < 40 + margin || s.y > H - 40 - margin) {
            const cx = W/2, cy=H/2; s.angle = Math.atan2(cy - s.y, cx - s.x) + rnd(-0.25,0.25);
          }
        }
    ...
    function onMove(ev: PointerEvent) {
      if (hoverLock) return; // 锁定后不变位置
      const rect = cvs.getBoundingClientRect();
      const x = ev.clientX - rect.left, y = ev.clientY - rect.top;
      let found: null | { id: string; x: number; y: number } = null;
      for (const s of spritesRef.current) {
        const k = sizeFactor(s.created_at), bw = s.w*k, bh = s.h*k;
        if (x > s.x - bw/2 && x < s.x + bw/2 && y > s.y - bh/2 && y < s.y + bh/2) { found = { id: s.id, x, y }; break; }
      }
      setHovered(found);
    }
    ...
    return () => { cancelAnimationFrame(rafId); window.removeEventListener('resize', onResize); cvs.removeEventListener('pointermove', onMove); };
  }, [hoverLock]);
  ...
  async function reelUp() {
    ...
    if (f.caughtId) {
      ...
      if (res.ok) {
        ...
        await refreshAll(); try { window.dispatchEvent(new CustomEvent('pond:refresh')); } catch {}
        showToast('收到一条鱼，已加入你的收获！');
      } else { showToast('这条鱼已被别人抢先钓走了 :('); await refreshAll(); }
    }
  ...
  async function reactToFish(id: string, value: 1 | -1) {
    const prev = myReactions[id] || 0;               // 0、1、-1
    const next: 0 | 1 | -1 = (prev === value) ? 0 : value;  // 同值=取消；否则切换

    // —— 乐观更新：计数 & 我的状态 —— //
    setMyReactions((m) => ({ ...m, [id]: next }));
    setPondFish((list) => list.map((f) => {
      if (f.id !== id) return f;
      let likes = f.likes, dislikes = f.dislikes;
      if (prev === 1) likes -= 1;
      if (prev === -1) dislikes -= 1;
      if (next === 1) likes += 1;
      if (next === -1) dislikes += 1;
      return { ...f, likes: Math.max(0, likes), dislikes: Math.max(0, dislikes) };
    }));
    const s = spritesRef.current.find((x) => x.id === id);
    if (s) {
      if (prev === 1) s.likes -= 1;
      if (prev === -1) s.dislikes -= 1;
      if (next === 1) s.likes += 1;
      if (next === -1) s.dislikes += 1;
      s.likes = Math.max(0, s.likes); s.dislikes = Math.max(0, s.dislikes);
    }

    // —— 发请求：交给后端判定（北京时间 4 点边界） —— //
    try {
      const res = await fetch('/api/reaction', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fishId: id, value }) });
      if (!res.ok) throw new Error('bad');
      const j = await res.json();
      if (!j?.ok) throw new Error('bad');

      // 以服务端返回计数 & 状态为准校正
      setPondFish((list) => list.map((f) => f.id === id ? { ...f, likes: j.likes ?? f.likes, dislikes: j.dislikes ?? f.dislikes } : f));
      setMyReactions((m) => ({ ...m, [id]: (j.my_reaction as 0|1|-1) ?? next }));
    } catch {
      // 回滚为服务端真实数据
      await refreshAll();
      showToast('提交失败，已恢复');
  ...
  // 定时无感刷新
  useEffect(() => {
    const tick = () => { refreshAll().then(() => { try { window.dispatchEvent(new CustomEvent('pond:refresh')); } catch {} }); };
    const iv = setInterval(tick, 5000);
    const onVis = () => { if (document.visibilityState === 'visible') tick(); };
    window.addEventListener('visibilitychange', onVis);
    window.addEventListener('focus', tick);
    return () => { clearInterval(iv); window.removeEventListener('visibilitychange', onVis); window.removeEventListener('focus', tick); };
  }, []);
    ...
    if (s) {
      ...
      hoverCard = (
        ...
        >
          ...
          <div className="muted" style={{ marginBottom: 6 }}>作者：{s.owner_name}</div>
          <div className="muted" style={{ marginBottom: 8 }}>已存活：{d} 天 {h} 小时 {m} 分</div>
          <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
            <button
              className="ghost"
              onClick={(e) => { e.stopPropagation(); reactToFish(s.id, 1); }}
              style={{ borderColor: my === 1 ? '#ffd166' : undefined, background: my === 1 ? 'rgba(255,209,102,.15)' : undefined }}
              title={my === 1 ? '再次点击可取消' : '点赞'}
            >👍 {s.likes}</button>
            <button
              className="ghost"
              onClick={(e) => { e.stopPropagation(); reactToFish(s.id, -1); }}
              style={{ borderColor: my === -1 ? '#ff6b6b' : undefined, background: my === -1 ? 'rgba(255,107,107,.15)' : undefined }}
              title={my === -1 ? '再次点击可取消' : '点踩'}
            >👎 {s.dislikes}</button>
          </div>
        </div>
      );
    ...
    <div>
      <header style={{ display: 'flex', gap: 8, alignItems: 'center', padding: 8, borderBottom: '1px solid rgba(255,255,255,.08)' }}>
        <button className="ghost" onClick={() => {
          const c = drawCanvasRef.current as any; if (c) { c._strokes = []; c.redraw && c.redraw(); }
      ...
      <dialog ref={drawDlgRef} style={{ border: '1px solid rgba(255,255,255,.12)', borderRadius: 12,
        background: 'linear-gradient(180deg,#0f2236,#0d1e2f)', color: '#cfeaff', width: 'min(940px,95vw)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 12, padding: 12 }}>
          <canvas ref={drawCanvasRef} style={{ width: EXPORT_W, height: EXPORT_H, background: '#0b1a23', border: '1px solid rgba(255,255,255,.12)', borderRadius: 8, display: 'block' }} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,28px)', gap: 6 }}>
                {palette.map((c) => (
                  <button key={c} onClick={() => setCurrentColor(c)} title={c}
                    style={{ width: 28, height: 28, borderRadius: 8, border: `2px solid ${currentColor === c ? '#fff' : 'rgba(255,255,255,.25)'}`, background: c }} />
                ))}
              </div>
            </div>
          </div>
...
function setupHiDPI(canvas: HTMLCanvasElement, w?: number, h?: number) {
  function resize() {
    ...
    const rectW = Math.floor(rect.width) || 0, rectH = Math.floor(rect.height) || 0;
